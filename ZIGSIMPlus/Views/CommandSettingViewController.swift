...
public class CommandSettingViewController : UIViewController {
    ...
    private func updateSettingTextData() -> Bool {
        ...
        for textField in textFields {
            if textField.tag == 0 {
                if Utils.checkSettingText(text: textField, settingTextType: .IP_ADDRESS) && "" != textField.text {
                    texts[.ipAdress] = textField.text ?? ""
                } else {
                    return false
                }
            } else if textField.tag == 1 {
                if Utils.checkSettingText(text: textField, settingTextType: .PORT_NUMBER) && "" != textField.text {
                    texts[.portNumber] = textField.text ?? ""
                } else {
                    return false
                }
            } else if textField.tag == 2 {
                if Utils.checkSettingText(text: textField, settingTextType: .DEVICE_UUID) && "" != textField.text {
